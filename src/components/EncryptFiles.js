...
export default function EncryptFiles1({ username }) {
  ...
  const handleEncrypt = async e => {
    e.preventDefault();
    let filename = customFilename || `TextEntry_${new Date().toISOString()}`;
    let content = text;
    try {
      const res = await fetch('http://localhost:5000/api/encrypt-file', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ username, filename, filetype: "text/plain", content })
      });
      if (!res.ok) throw new Error("Encryption failure");
      alert('Text encrypted and stored!');
      setText(''); setCustomFilename('');
    } catch (err) {
    ...
    <PageWrapper>
      ...
      <Section>
        ...
        <form onSubmit={handleEncrypt}>
          <Input
            type="text"
            placeholder="Enter filename for text..."
